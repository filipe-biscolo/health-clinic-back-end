  ...
  async create(request: Request, response: Response) {
    ...
    let schedule = await scheduleRepository.find({
      where: {
        ...
        date_hour: Between(date_hour, date_hour_end),
      },
    });
    ...
    if (!schedule || schedule.length === 0) {
      schedule = await scheduleRepository.find({
        where: {
          clinic_id: clinic_id,
        professional_id: professional_id,
        date_hour_end: Between(date_hour, date_hour_end),
        },
      });
    }
  ...
  async update(request: Request, response: Response) {
    ...
    let schedule = await scheduleRepository.find({
      where: {
        ...
        date_hour: Between(data.date_hour, data.date_hour_end),
      },
    });
    ...
    if (!schedule || schedule.length === 0) {
      schedule = await scheduleRepository.find({
        where: {
          ...
          clinic_id: data.clinic_id,
          professional_id: data.professional_id,
          date_hour_end: Between(data.date_hour, data.date_hour_end),
        },
      });
    }
